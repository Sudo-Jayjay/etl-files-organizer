...
def sql_file_to_excel(conn_str, sql_filepath, output_folder):
    ...
    with engine.connect() as conn:
        ...
        for i, stmt in enumerate(statements, start=1):
            ...
            if is_query:
                ...
                xlsx_count += 1
                df.to_excel(f"{output_folder}/output_{xlsx_count}.xlsx", index=False)
            else:
                conn.execute(text(stmt))
        conn.commit()
        print(f"Total statements: {len(statements)}, Excel files created: {xlsx_count}")